...
Дата: 2026-08-20. Статус: **код-миграция сделана**, принято как
[ADR 0008](./adr/0008-llama-cpp-pro-migration.md) — `package.json` (оба репозитория) и адаптер уже на
`llama-cpp-pro@^0.2.4`. Не закрыто: device-верификация из §7 (session-cache тайминг, APK-size,
smoke-прогон, регрессия jinja-бага и `n_predict=50`-бага) — реального устройства под неё ещё не было.
Живёт в `local-ai`, потому что весь физический объём
