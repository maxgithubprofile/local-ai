...
### iOS token-streaming patch merged into forta.chat's build, still not device-verified (2026-09-03)

**Context:** User asked to get the drafted iOS patch above actually into forta.chat's build ("сделай
ios патч в сборке"), explicit caveat that they couldn't verify it on a device right now.

**What changed, in `forta.chat`'s own repo (`c:\inetpub2025\forta.chat`), not this one:** rebased the
patch's paths from `ios/Sources/...` to `node_modules/llama-cpp-pro/ios/Sources/...` (the prefix
`patch-package` diffs use) and appended it to the end of `forta.chat/patches/llama-cpp-pro+0.2.4.patch`
— the same file that already carried the Android per-token-streaming fix and the UTF-8 streaming fix,
per this file's convention for multiple patches against one package. Applied it for real (not just
dry-run) against forta.chat's actual installed `node_modules/llama-cpp-pro`, verified the resulting
Swift files contain `onPartialToken`/`runCompletionStream`/`notifyListeners("@LlamaCpp_onToken"...)`,
then confirmed it also reverts cleanly (`patch -p1 -R`) — so the diff is a correct, reversible
transformation of the real files it targets, not just of the drafted copy in this repo.

**Still exactly as unverified as before at the behavior level** — nothing here compiled Swift, built
the xcframework, or ran on a device/simulator. Every `TODO(mac)` from the previous entry is still open.
"In the build" means `patch-package`'s `postinstall` will apply it on every install; it does not mean
it has been shown to work at runtime.

**Follow-up the same session — real drift found and fixed, tracked patch file regenerated:** dry-run
applying forta.chat's (then hand-spliced) patch file against a freshly-downloaded pristine
`llama-cpp-pro@0.2.4` failed 3 of the pre-existing Android hunks in `LlamaCpp.java`. Root-caused, not
just logged: the pristine tarball's integrity hash matched forta.chat's own `package-lock.json`/
`yarn.lock` exactly (`sha512-CPGajT...`, byte-identical — ruled out a registry/mirror mismatch), but
`node_modules/llama-cpp-pro`'s **live** `LlamaCpp.java` called `completionNative(context.getNativeContextId(),
contextId, params)` — a 3-argument call, `contextId` added — while the tracked patch file only ever
produced the older 2-argument call. Someone had hand-edited `node_modules/llama-cpp-pro` directly
(added `contextId` plumbing to `completionNative`'s native signature and all three call sites, almost
certainly the real fix needed to route streamed tokens to the right JS-side context on Android) and
the working, device-verified fix was **never captured back into `patches/llama-cpp-pro+0.2.4.patch`**.
A genuinely fresh install anywhere else would have silently lost that fix.

**Fixed by regenerating the patch from the live tree**, the way `patch-package`'s own error message
recommends: `patch-package`'s auto-diff (`npx patch-package llama-cpp-pro`) initially failed for an
unrelated reason — `node_modules/llama-cpp-pro/android/{build,.cxx}` (513MB of gitignored Gradle build
cache, not source) had file paths too long for its internal `git add` on Windows. Deleting those two
directories (safe: build output, regenerated by Gradle, not tracked) let the auto-diff complete
successfully, producing a fresh `patches/llama-cpp-pro+0.2.4.patch` diffed directly against a real
pristine copy — capturing the Android streaming + `contextId` fix, the UTF-8 streaming fix, and this
session's iOS patch all in one correct file, superseding the earlier hand-spliced one.

**Validated for real, not just dry-run:** deleted `node_modules/llama-cpp-pro` entirely, reinstalled
`llama-cpp-pro@0.2.4` fresh from the registry (`--ignore-scripts`, confirmed zero patch markers
present), then ran plain `npx patch-package` (the same command `postinstall` runs) — applied cleanly,
`llama-cpp-pro@0.2.4 ✔`, and all three fixes' markers (`onPartialToken`, the 3-arg `completionNative`,
`format_token_utf8_safe`) present afterward. This is the authoritative test (it's `patch-package`'s own
apply mechanism, not shell `patch`, which is what actually runs in `postinstall`) — the GNU-`patch`
dry-run failures earlier were a Windows CRLF-handling artifact of that diagnostic tool, not a real
defect in the tracked patch. The `contextId` drift was real and is now fixed; the CRLF-related hunk
failures were not.

**Process takeaway, worth keeping:** never hand-edit files under `node_modules/<package>` and leave it
at that — the fix only survives a teammate's `npm install` or a CI runner's clean checkout if
`npx patch-package <package>` is re-run immediately after, regenerating the tracked patch file from
the edited tree. `forta.chat`'s three coexisting lockfiles (`package-lock.json`, `pnpm-lock.yaml` —
...
device-verified fix that lived only in `node_modules`, invisible to a fresh install because nobody had
re-run `patch-package` after hand-editing it — was treated as a symptom of `patch-package` itself being
a poor fit for a dependency with this much native-code drift, not a one-off mistake to just fix and
move on from. User agreed to fork rather than keep patching.

**What exists now:** `github.com/maxgithubprofile/llama-cpp-pro`, forked from upstream
`arusatech/llama-cpp-pro` (MIT). Its history: a `vendor: reset to published npm
llama-cpp-pro@0.2.4 content` commit (working tree replaced with the registry tarball's content,
verified byte-identical via its `sha512-CPGajT...` integrity hash — same one pinned in forta.chat's
`package-lock.json`/`yarn.lock`), then a `fix: per-token streaming (Android + iOS) and UTF-8 streaming
safety` commit carrying everything the old patch file did (Android `notifyListeners`/`emitTokenEvent`
wiring + the `contextId` fix, `cap-ios-bridge.cpp`'s `format_token_utf8_safe`, and the iOS
`runCompletionStream`/`onPartialToken`/`notifyListeners` patch from two entries above). Tagged
`v0.2.4-local-ai.1`.

**Why not just fork `main` and patch that:** upstream's `main` had already moved past a structure
compatible with the published `0.2.4` — it replaced the flat `cpp/` directory with `native/` plus a
`third_party/llama.cpp` git submodule (`CMakeLists.txt` there literally says "Legacy flat cpp/ is not
used"). Diffing the old patch against `main` failed for real, structural reasons, not the CRLF
red herring below. The fork's baseline is therefore the *published tarball content*, not `main`'s tip
— meaning a future sync with upstream needs deliberate re-porting against `native/`, not a plain
`git merge`. That was already true before the fork existed (the same incompatibility would have hit
anyone trying to patch `main` directly), so this isn't a regression the fork introduced.

**A real, reproducible tooling gotcha hit and worked around:** applying the full combined patch via
GNU `patch -p1` failed the same 5 hunks (`LlamaCpp.java` ×3, `jni.cpp` ×2) against *three independent,
verified-identical* copies of the pristine source (a fresh `npm pack`, upstream's tip once CRLF-
normalized, and this fork's own vendor-reset commit) — while `patch-package`'s own JS-based apply
succeeded against the same content every time (verified end-to-end in the previous entry: delete
`node_modules/llama-cpp-pro`, fresh install, `npx patch-package`, fixes present). Conclusion: this is a
genuine bug/quirk in this environment's GNU `patch` on hunks with blank-line-only context, not a
content problem — worked around here by skipping `patch` entirely and copying the already-validated,
already-correct files from forta.chat's real `node_modules/llama-cpp-pro` straight into the fork,
rather than re-deriving them through a tool that had just been shown unreliable for this exact diff,
three times, on this machine.

**Follow-up still needed, not done in this session:** update `forta.chat/package.json`'s
`"llama-cpp-pro": "^0.2.4"` to
`"llama-cpp-pro": "github:maxgithubprofile/llama-cpp-pro#v0.2.4-local-ai.1"`, delete
`patches/llama-cpp-pro+0.2.4.patch` (re-applying it on top of the fork's already-fixed source would
fail), and verify a real `npm install` resolves and builds the git dependency correctly — the fork's
`prepare` script (`scripts/prepare-js-dist.cjs`) already exists specifically to build `dist/` on a git
install, so this should work without new tooling, but hasn't been end-to-end verified in forta.chat
itself yet.
